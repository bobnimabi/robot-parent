...
public class LoginServer extends FunctionBase<LoginDTO> {
    ...
    protected ResponseResult doFunctionFinal(ParamWrapper<LoginDTO> paramWrapper, RobotWrapper robotWrapper, TenantRobotAction action) throws Exception {
        ...
        StanderHttpResponse standerHttpResponse = execute.request(robotWrapper, CustomHttpMethod.GET, action, null, createLoginParams(robotWrapper, loginDTO), null, LoginParse.INSTANCE, false);
    ...
    private ICustomEntity createLoginParams(RobotWrapper robot, LoginDTO loginDTO) {
        ...
        return UrlCustomEntity.custom()
                ...
                .add("redirectURL", "http://iservice.10010.com/e4/skip.html?menuCode=000100020001")
    ...
    private static final class LoginParse implements IResultParse {
        ...
        public ResponseResult parse(String result) {
            ...
            if (Constant.SUCCESS.equals(resultCode)) {
                return ResponseResult.SUCCESS("登录，成功");
            }
            return ResponseResult.FAIL("登录,失败：" + msg);
        }
    }
}
